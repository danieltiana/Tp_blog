...
function Home_Article() {
  ...
  return (
    <section id="articles " className="py-5 bg-transparent bg-gradient-to-r from-black to-gray-900 bg-opacity-50 opacity-0 transition-opacity-duration-1000 ease-out opacity-100">
      <Container data-aos="fade-up">
        <h2 className="text-center mb-5 mon-titre font-bold bg-gradient-to-r from-blue-400 to-purple-400 text-transparent bg-clip-text text-5xl">Nos articles</h2>
        <Row>
          {echantillon.map((article) => (
            <Col key={article.id} md={4} className="mb-4">
              ...
              >
                ...
                <Card.Img variant="top" className="opacity-40 hover:opacity-60 transition-opacity" src={article.image} />
                <Card.Body>
                  <Card.Title className="text-warning">{article.titre}</Card.Title>
                  <Card.Text className="text-secondary">{`${article.descriptions.length <= 70 ? article.descriptions : article.descriptions.slice(0, 90) + "..."}`}</Card.Text>
